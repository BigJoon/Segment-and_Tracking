...
class SimpleSegTracker():
    def __init__(self, segtracker_args, sam_args, aot_args) -> None:
        """
         Initialize SAM and AOT (without detector for simplified version).
        """
        self.sam = Segmentor(sam_args)
        self.tracker = get_aot(aot_args)
        # Skip detector initialization to avoid GroundingDINO issues
        self.detector = None
        ...
        print("SimpleSegTracker has been initialized (without text detection)")
